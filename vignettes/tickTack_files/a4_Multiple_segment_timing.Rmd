...
vignette: >
  %\VignetteIndexEntry{Timing Clonal Peaks in a hierarchical fashioin}
...
p_elbo <- list()
for (i in 1:K){
  p_elbo[[i]] <- tickTack::plot_elbo_h(elbo_iterations, i) + ggtitle(paste0("K = ", i))
}
...
p_model_selection <- plot_model_selection_h(model_selection_tibble, best_K)
p_model_selection
...
for (i in 1:K){
  plot_model_selection_inference[[i]] <- tickTack::plot_timing_h(results, i) + ggtitle(paste0("K = ", i))
}
