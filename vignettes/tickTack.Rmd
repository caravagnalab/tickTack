...
tickTack requires in input a CNAqc object with attributes cna, mutations and metadata. 
...
<img src="timing_problem.png" alt="Overview timing problem" width="650"/><br>
...
<img src="Mutations_single_double_copy_draw_process.png" alt="Overview timing problem" width="650"/><br>
...
data_reduced <- list(mutations = tibble( data$mutations %>% filter(chr %in% paste0("chr",c(1:4)))), 
                     cna = tibble( data$cna %>% filter(chr %in% paste0("chr",c(1:4)))),
...
p <- plot_inference_h(results, data_reduced$cna, input_data, colour_by = "karyotype", 3)
p
