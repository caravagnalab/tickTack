...
#' @param single_k perform a single inference with only K components
#'
#' @return   results_and_data = list(data = input_data_list, results = results, output_files_list = output_files_list)
#' @export
fit_h_single_k = function(x, single_k, max_attempts=2, INIT=TRUE, tolerance = 0.0001, possible_k = c("2:1", "2:2", "2:0"), alpha = .05, min_mutations_number = 2, n_components = 0, initial_iter=200, grad_samples=10, elbo_samples=200)
{
  ...
  # before inference add K to the list obtained as input_data
    ...
    names_tau <- paste("tau[", 1:single_k, "]", sep = "")
    names_weights <- outer(1:S, 1:single_k,
    ...
    clock_assignment <- compute_clock_assignment(accepted_cna, tau_and_w_draws, tau_and_w_summary, single_k, data_simulation=NULL)
    ...
    elbo_iterations[[single_k]] = elbo_df
