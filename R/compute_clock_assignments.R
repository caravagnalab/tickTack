compute_clock_assignment <- function(accepted_cna, tau_and_w_draws, tau_and_w_summary, K, data_simulation=NULL){
  ...
  tau_inferred_medianap <- lapply(1:ncol(tau_inferred), function(i) {median(tau_inferred[,i])} ) %>% unlist() 
  
  tau_original_list = c()
  tau_inferred_assigned_list = tibble()
  ...
  for (i in 1:nrow(accepted_cna)){
    ...
    weights_inferred_draws <- draws_and_summary[[K]]$draws
    weights_inferred <- weights_inferred_draws[, colnames(weights_inferred_draws) %in% names_weights]
    ...
    tau_inferred_single <- tibble (tau_inferred_low = tau_inferred_low,tau_inferred_median = tau_inferred_assigned, tau_inferred_high = tau_inferred_high)
    # tau_inferred_assigned_list <- c(tau_inferred_assigned_list, tau_inferred_single)
    tau_inferred_assigned_list <- bind_rows(tau_inferred_assigned_list, tau_inferred_single)
