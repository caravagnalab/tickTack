...
#' @export
#' 
#' @examples results <- fit(x, max_attempts = max_attempts,INIT = INIT, tolerance = tolerance )
#' fit()

fit = function(x, max_attempts=2, INIT=TRUE, tolerance = 0.01, possible_k = c("2:1", "2:2", "2:0"), alpha = .05, min_mutations_number = 2)
{
  ...
  for (K in 1:k_max){
    ...
    if (INIT==TRUE){
      ...
      res <- fit_variational(input_data, 
    ...
    log_lik_matrix <- extract_log_lik(stanfit, parameter_name = "log_lik", merge_chains = TRUE) # merge chains potrebbe non servire  #getter?
