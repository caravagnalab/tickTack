...
predict_best_number_of_clusters <- function(fit, purity, coverage){
  ...
  model_path <- system.file("xgboost", "xgboost_model_selection.rds", package = "tickTack", mustWork = TRUE)
  ...
  features <- c("BIC_w", "ICL_w", "rank_AIC","rank_ICL","rank_LOO", "rank_BIC", 
                ...
                "mean_density_mut","mutations_density",
  ...
  df <- tbl_info %>%
    dplyr::mutate(mutations_density = .data$mean_density_mut) %>% 
    dplyr::mutate(min_AIC = min(.data$AIC),
                  min_BIC = min(.data$BIC),
                  min_ICL = min(.data$ICL),
                  min_LOO = max(.data$LOO),
                  rank_AIC = dplyr::dense_rank(.data$AIC),
                  rank_BIC = dplyr::dense_rank(.data$BIC),
                  rank_ICL = dplyr::dense_rank((.data$ICL)),
                  rank_LOO = dplyr::dense_rank(dplyr::desc(.data$LOO))         
    ) %>%
    dplyr::ungroup()
  
  # df_single_sample <- df_single_sample %>%
  #   mutate(scores = map2(real_clock, inferred_clock, score_clocks)) %>%
  #   unnest_wider(scores)
  
  df <- df %>%
    dplyr::mutate(score_geom = (.data$min_cluster_separation) - (.data$max_cluster_overlap) ) %>%
    dplyr::mutate(max_score = max(.data$score_geom))
  
  df <- df %>%
    dplyr::rowwise() %>%
    dplyr::mutate(
      clock_vals = list(as.numeric(strsplit(.data$inferred_clock, "_")[[1]])),
      clock_low  = list(as.numeric(strsplit(.data$inferred_low, "_")[[1]])),
      clock_high = list(as.numeric(strsplit(.data$inferred_high, "_")[[1]])),
      clock_sd   = stats::sd(.data$clock_vals),
  ...
  df_single_sample <- df %>%
    dplyr::mutate(
      BIC_w = .data$BIC * 1,   # downweight BIC
      ICL_w = .data$ICL * 50      # upweight ICL
    ) %>% dplyr::mutate (coverage = coverage, 
                  purity = purity, 
                  n_cna = fit$results$data$accepted_cna %>% nrow())
...
process_file_prediction <- function(fit) {
  tryCatch({
    ...
    cna <- results$data$accepted_cna %>%
      dplyr::rowwise() %>%
      dplyr::mutate( from = strsplit(.data$segment_name,split="_")[[1]][2], to = strsplit(.data$segment_name,split="_")[[1]][3]) %>%
      dplyr::mutate( len = as.numeric(.data$to) - as.numeric(.data$from))
    ...
    for (i in seq_len(Kmax)){
      ...
      unique_intervals <- unique(best_fit %>%
                                   dplyr::select(.data$clock_low, .data$clock_high, .data$clock_mean))
      ...
      intervals <- unique_intervals %>%
        dplyr::mutate(
          mean = .data$clock_mean,
          low  = .data$clock_low,
          high = .data$clock_high
        )
